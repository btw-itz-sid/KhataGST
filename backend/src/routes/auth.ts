...
// ✅ OTP cooldown: 60s per phone
// ✅ Brute force protection: max 5 wrong attempts
// ✅ JWT expiry: 7 days (was 30d)
...
/**
 * Check if phone is on cooldown (last OTP sent < 60s ago)
 */
async function checkOtpCooldown(phone: string): Promise<{ allowed: boolean; retryAfterSec?: number }> {
  ...
  const elapsedSec = (Date.now() - lastCreated) / 1000;

  if (elapsedSec < OTP_COOLDOWN_SECONDS) {
    const retryAfterSec = Math.ceil(OTP_COOLDOWN_SECONDS - elapsedSec);
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/send-otp", async (request, reply) => {
    ...
    // ── OTP Cooldown Check ──────────────────────────────────────────────
    try {
      const cooldown = await checkOtpCooldown(phone);
      if (!cooldown.allowed) {
        return reply.status(429).send({
          success: false,
          error: {
            code: "OTP_COOLDOWN",
            message: `Thoda ruko — ${cooldown.retryAfterSec}s baad try karo`,
            retry_after: cooldown.retryAfterSec,
          },
        });
      }
    } catch (err) {
      console.error("OTP cooldown check failed:", err);
      // Don't block if cooldown check fails — proceed with send
    }
  ...
  app.post(
    ...
    async (_request, reply) => {
      ...
      // Future: add token blacklist with Redis
      return reply.send({
        success: true,
        message: "Logged out — token client se remove karo",
      });
    }
  );
}
