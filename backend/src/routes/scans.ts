...
export async function scanRoutes(app: FastifyInstance) {
  ...
  app.post("/", { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    // Upload via Storage Service (Cloud R2/S3 or local persistence)
    ...
    const filePath = path.join(UPLOADS_DIR, uploadResult.storageKey);
    ...
    const scanRecord = await query(
      `INSERT INTO bill_scans (business_id, uploaded_by, file_url, storage_provider, storage_key, status)
       VALUES ($1, $2, $3, $4, $5, 'processing') RETURNING id`,
      [business_id, userId, uploadResult.url, uploadResult.storageProvider, uploadResult.storageKey]
    );
